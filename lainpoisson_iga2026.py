# -*- coding: utf-8 -*-
"""LainPoisson_IGA2026.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1vhpC23CAchhrHAYWsELVSPyWvTERa0Hh
"""
...
EXAMPLE = 1 #1 sin*sin, #2 exp*sin #3 Eriksson-Johnson
...
def exact_solution_dx2(x, y) -> torch.Tensor:
  if EXAMPLE==1:
  # u(x,y) = sin(2*pi*x)sin(2.0*pi*y)
    sins = -4*torch.pi*torch.p*torch.sin(2*torch.pi*x) * torch.sin(2.0*torch.pi*y)
    res = sins
    return res
  if EXAMPLE==2:
...
def exact_solution_dt2(x, y) -> torch.Tensor:
  if EXAMPLE==1:
  # u(x,y) = sin(2*pi*x)sin(2.0*pi*y)
    sins = 4*torch.pi*torch.pi*torch.sin(2*torch.pi*x) * torch.sin(2.0*torch.pi*y)
    res = sins
    return res
  if EXAMPLE==2:
...
def compute_norms(pinn: PINN, x: torch.Tensor, y: torch.Tensor):
    ...
    with open('OutputPlots.txt', 'a') as file_f:
       print(f"||uNN-uexact||h1: {h1_norm:.7f} loss approx: {loss_approximation:.7f}", file=file_f)
...
def interior_loss(pinn: PINN, x:torch.Tensor, y: torch.tensor):
    if EXAMPLE==1:
      f1 = -4.0*torch.pi*torch.pi*torch.sin(2.0*torch.pi*x)*torch.sin(2.0*torch.pi*y)
      f2 = -4.0*torch.pi*torch.pi*torch.sin(2.0*torch.pi*x)*torch.sin(2.0*torch.pi*y)
      rhs = -f1-f2 # -Delta u = f so f = -Delta u, 0 on boundary, so the residual will be res = Delta u+f
    ...
    if EXAMPLE==3:
      loss = dfdt(pinn, x, y, order=1) - EPSILON*dfdt(pinn, x, y, order=2) - EPSILON*dfdx(pinn, x, y, order=2)
      loss += shift_EJ_dt(x, y) - EPSILON*shift_EJ_dt2(x, y) - EPSILON*shift_EJ_dx2(x, y)
    ...
    for epoch in range(max_epochs):
        try:
            ...
            if (epoch + 1) % 1000 == 0:
                ...
                error_h1 = NORMS_H1[-1]
                print(f"Epoch: {epoch + 1} - Loss: {float(loss):>7f}; error H1: {error_h1}; total runtime: {time_total}; time per epoch: {time_total / epoch};")
                time_total = 0
...
def plot_color_exact(z: torch.Tensor, x: torch.Tensor, t: torch.Tensor, n_points_x, n_points_t, figsize=(8, 6), dpi=100):
    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
    z_raw = z.detach().cpu().numpy()
    x_raw = x.detach().cpu().numpy()
    t_raw = t.detach().cpu().numpy()
...
def plot_color_error(z: torch.Tensor, x: torch.Tensor, t: torch.Tensor, n_points_x, n_points_t, figsize=(8, 6), dpi=100):
    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
    z_raw = z.detach().cpu().numpy()
    x_raw = x.detach().cpu().numpy()
    t_raw = t.detach().cpu().numpy()
...
# Define an activation fxn as ratio of two polynomials with coefficients that are learned during gradient descent
class LearnableRational(nn.Module):
    def __init__(self, num_order=2, den_order=2, eps=1e-6):
        """
        A learnable activation function expressed as a rational function:
            f(x) = (a0 + a1*x + ... + a_{n} * x^{n}) / (1 + b1*x + ... + b_{m} * x^{m})
        Here, the denominator always starts with 1 to avoid scale ambiguity.
        Both the numerator coefficients and the denominator coefficients (except the constant 1)
        are initialized randomly and then learned via gradient descent.

        Args:
            num_order (int): Degree of the numerator polynomial.
            den_order (int): Degree of the denominator polynomial.
            eps (float): A small epsilon added for numerical stability
        """
        super().__init__()
        self.eps = eps

        # Numerator parameters: a0, a1, ..., a_num_order (total num_order+1 coefficients)
        self.num_coeffs = nn.Parameter(torch.randn(num_order + 1))
        # Denominator parameters: b1, ..., b_den_order (we fix the constant term to 1)
        self.den_coeffs = nn.Parameter(torch.randn(den_order))

    def forward(self, x):
        # Compute the numerator: sum_{i=0}^{num_order} a_i * x^i
        num = 0.0
        for i, coeff in enumerate(self.num_coeffs):
            num = num + coeff * (x ** i)

        # Compute the denominator: 1 + sum_{i=1}^{den_order} b_i * x^i
        den = 1.0
        for i, coeff in enumerate(self.den_coeffs, start=1):
            den = den + coeff * (x ** i)
        # Optionally add a small eps for stability (or work with abs(den) if needed)
        den = torch.clamp(den, min=self.eps)
        return num / den

    def extra_repr(self):
        return f'num_coeffs={self.num_coeffs.data}, den_coeffs={self.den_coeffs.data}'

pinn = PINN(LAYERS, NEURONS_PER_LAYER, pinning=True, act=LearnableRational(num_order=2, den_order=2)).to(device)
