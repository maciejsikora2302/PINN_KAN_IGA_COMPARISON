...
def plot_trajectory_overlays(runs, save_dir):
    """
    1. Convergence Trajectory Overlays (Loss & H1 error vs. Epochs):
       For each problem/scenario, overlay PINN vs. R-PINN vs. IGA-KANN vs. R-IGA-KANN on a single semi-log plot.
       Include horizontal dashed reference lines for IGA-SUPG and IGA-RM final errors.
    """
    print("Generating Figure 1: Convergence Trajectory Overlays...")
    os.makedirs(save_dir, exist_ok=True)
    
    # Let's group by Example (1: Sine, 2: Exp, 3: Eriksson-Johnson)
    examples = {
        1: "Poisson Sine (Ex 1)",
        2: "Poisson Exp (Ex 2)",
    ...
    for ex_id, title in examples.items():
        ...
        # We need to find the runs for this example
        # Let's filter runs where meta["config"]["EXAMPLE"] == ex_id
        ex_runs = []
        ...
        # Draw Trajectory
        # PINN, R-PINN, IGA-KANN, R-IGA-KANN
        # Note: IGA-KANN is KAN with RPINN=0, R-IGA-KANN is KAN with RPINN=1
        h1_calc_every = 100
        ...
        for run in ex_runs:
            ...
            if pinn is not None and "loss_history" in pinn.files:
                ...
                epochs_loss = np.arange(len(loss_hist))
                epochs_h1 = np.arange(len(h1_hist)) * h1_calc_every
                
                axes[0].semilogy(epochs_loss, loss_hist, label=f"{suffix}PINN", alpha=0.8)
                if len(h1_hist) > 0:
                    axes[1].semilogy(epochs_h1, h1_hist, label=f"{suffix}PINN", alpha=0.8)
                has_plots = True
            ...
            if kan is not None and "loss_history" in kan.files:
                ...
                axes[0].semilogy(epochs_loss, loss_hist, label=f"{suffix}IGA-KANN", alpha=0.8)
                if len(h1_hist) > 0:
                    axes[1].semilogy(epochs_h1, h1_hist, label=f"{suffix}IGA-KANN", alpha=0.8)
                has_plots = True
                
        # Draw horizontal lines for IGA-SUPG and IGA-RM final errors
        # Let's search if we have any runs with IGA supg or igrm for this example
        for run in ex_runs:
            results = run["meta"].get("results", {})
            for solver, info in results.items():
                if solver == "IGA":
                    method = info.get("method", "standard")
                    h1_err = info.get("final_h1_error")
                    if h1_err is not None and method in ("supg", "igrm", "standard"):
                        axes[1].axhline(y=h1_err, linestyle="--", alpha=0.7, label=f"IGA-{method.upper()} Ref")
        ...
        if has_plots:
            ...
            axes[1].set_title("H1 Error vs. Epochs")
            axes[1].set_xlabel("Epochs")
            axes[1].set_ylabel("H1 Error")
            axes[1].grid(True, which="both", linestyle="--", alpha=0.5)
            axes[1].legend()
            
            fig.suptitle(f"Convergence Trajectories - {title}", fontsize=14)
            plt.tight_layout()
            plt.savefig(os.path.join(save_dir, f"trajectory_ex{ex_id}.png"), dpi=300, bbox_inches='tight')
        plt.close()
