...
def calibrate(optimized: bool = False):
...
def estimate_time(config_path, pinn_base_coeff, pinn_lu_coeff, kan_coeff, iga_coeff, wall_time_limit_sec=None, optimized=False):
    # Load config
    config = SharedConfig()
    config.load_config(config_path)
    
    # Check active solvers
    active_solvers = [s.lower() for s in getattr(config, "SOLVERS", ["pinn", "kan", "iga"])]
    ...
    if "pinn" in active_solvers:
        ...
        if is_rpinn:
            cost_per_epoch_pinn += (N * pinn_lu_coeff if optimized else (N ** 2) * pinn_lu_coeff)
    ...
    if "kan" in active_solvers:
        ...
        if is_rpinn and optimized:
            cost_per_epoch_kan += N * pinn_lu_coeff
    ...
    if "iga" in active_solvers:
        if optimized:
            # Sum-factorization element integration complexity: M^2 * (p+1)^3
            ...
            # Standard element integration complexity: M^2 * (p+1)^6
            iga_inner_loops = (iga_elements ** 2) * ((iga_degree + 1) ** 6)
            iga_assembly_est = iga_inner_loops * iga_coeff
            iga_eval_est = N * ((iga_degree + 1) ** 2) * (iga_coeff * 0.5)
        iga_est = 0.1 + iga_assembly_est + iga_eval_est
...
def main():
    ...
    print(f"Mode: {mode_str} | Applying Wall-Clock Time Limit per Solver Run: {args.time} minutes ({wall_time_sec:.0f}s)")
    print("-" * 145)
    ...
    for c in configs:
        ...
        try:
            est = estimate_time(config_path, pinn_base_coeff, pinn_lu_coeff, kan_coeff, iga_coeff, wall_time_limit_sec=wall_time_sec, optimized=args.optimized)
